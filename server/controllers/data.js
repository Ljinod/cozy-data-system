...
var client, db, dbHelper, encryption, feed, git, mapDoc, selectDocPlug, selectUserPlug;
      ...
      if (doc != null) {
        ...
        return db.save(req.params.id, req.body, function(err, doc) {
          if (err) {
            ...
            mapDoc(req.body, doc.id, function(err, ids) {
              if (err) {
                return console.log('Error on the mapping : ' + err);
              } else if (ids != null) {
                if (ids[0] != null) {
                  console.log('new doc inserted : ' + ids[0]);
                }
                if (ids[1] != null) {
                  return console.log('new user inserted : ' + ids[1]);
                }
              }
            });
            return res.send(201, {
