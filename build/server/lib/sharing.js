...
module.exports.mapDocOnInsert = function(doc, id, callback) {
  return mapDocInRules(doc, id, function(err, results, shareid) {
    if (err || (results == null)) {
      return callback(err);
    } else {
      return async.series([
        function(_callback) {
          if (results[0]) {
            return plug.insertDoc(results[0].docid, shareid, results[0].userDesc, function(err) {
              if (err != null) {
                return callback(err);
              } else {
          ...
          if (results[1]) {
            return plug.insertUser(results[1].docid, shareid, results[1].userDesc, function(err) {
              if (err != null) {
                return callback(err);
              } else {
...
mapDocInRules = function(doc, id, callback) {
  return rules.forEach(function(rule) {
    ...
    ], function(err, filteredRes) {
      if ((filteredRes[0] != null) || (filteredRes[1] != null)) {
        console.log('got a mapping !! ' + JSON.stringify(filteredRes));
        console.log('share id : ' + rule.id);
      }
      return callback(err, filteredRes, rule.id);
    });
  });
};
